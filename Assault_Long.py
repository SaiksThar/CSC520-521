...
Y = Y.drop(['INCIDENT_NUMBER','OCCURRED_ON_DATE','Location','REPORTING_AREA'], axis =1)

Y['MONTH']= Y['MONTH'].astype(float)

Y['OFFENSE_CODE_GROUP'] = Y['OFFENSE_CODE_GROUP'].replace(['Manslaughter','Aggravated Assault','Simple Assault','Homicide','Robbery'],'Assault')

Y = Y[Y.OFFENSE_CODE_GROUP == 'Assault']

target_long = Y.iloc[:,[11]]
